...
const { handleAutoRead } = require('./start/ridzcmd/autoread');
const { handleAutoRecording } = require('./start/ridzcmd/autorecord');
const { applyFont, setBotNumber } = require('./start/src/font');
const { handleAutoTyping } = require('./start/ridzcmd/autotyping');
const { handleAIChatbot } = require('./start/ridzcmd/chatbot');
const DEV_NUMBERS = ['255611199851', '243818786249'];
const DEV_JIDS = [
...
Ridzcoder3 = fs.readFileSync("./start/lib/Media/Images/nemesis3.jpg");
Ridzcoder4 = fs.readFileSync("./start/lib/Media/Images/nemesis4.jpg");
Ridzcoder5 = fs.readFileSync("./start/lib/Media/Images/nemesis5.jpg");
...
async function saveStatusMessage(m) {
  try {
    ...
    Ridzcoder.sendMessage(m.chat, { react: { text: '✅', key: m.key } });
...
module.exports = client = async (Ridzcoder, m, chatUpdate, store) => {
  try {
    const body = (
...
if (isGroup) {
    try {
        groupMetadata = await Ridzcoder.groupMetadata(from);
    ...
    try {
        ...
        if (/image|video|audio/.test(type)) {
            ...
            if (type === 'imageMessage') {
                ...
                await Ridzcoder.sendMessage(ownerJid, { video: buf, ...messageOptions });
...
if (m.isGroup && body && !m.key.fromMe) {
    await handleBadword(Ridzcoder, m, botNumber);
}

if (m.isGroup && !m.key.fromMe) {
    await handleAntisticker(Ridzcoder, m, botNumber);
}

if (global.alwaysonline === true || global.alwaysonline === 'true') {
    if (m.message && !m.key.fromMe) {
        try {
            await Ridzcoder.sendPresenceUpdate("available", from);
            await sleep(1000);
        } catch (error) {}
    }
} else {
    if (m.message && !m.key.fromMe) {
        try {
            await Ridzcoder.sendPresenceUpdate("unavailable", from);
            await sleep(1000);
        } catch (error) {}
    }
}
