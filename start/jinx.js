...
async function handleAntiDelete(m, Ridzcoder) {
    try {
        ...
        if (deletedMsg.key.remoteJid === 'status@broadcast') {
            ...
            try {
                const groupInfo = await Ridzcoder.groupMetadata(chatId);
        ...
        if (antideleteSetting === 'private') {
            targetChat = Ridzcoder.user.id; // Bot owner's inbox
        ...
        if (!deletedMsg.message.conversation && !deletedMsg.message.extendedTextMessage) {
            try {
                let forwardedMsg = await Ridzcoder.sendMessage(
                    targetChat,
                    { 
                        forward: deletedMsg,
...
╰────────────────≽`;
                ...
                let quotedMessage = {
                    key: {
                        remoteJid: chatId,
                        fromMe: sender === Ridzcoder.user.id,
                        id: messageId,
                        participant: sender
                    },
...
async function handleAntiEdit(m, Ridzcoder) {
    try {
        // Get bot number
        const botNumber = await Ridzcoder.decodeJid(Ridzcoder.user.id);
...
╰────────────────≽`;
        ...
        if (antieditSetting === 'private') {
            targetChat = Ridzcoder.user.id; // Send to bot owner
...
async function handleLinkViolation(Ridzcoder, m, message, botNumber) {
    try {
        ...
        switch(mode) {
            case 'warn': {
                ...
                if (userWarnings.count >= 3) {
                    try {
                        await Ridzcoder.groupParticipantsUpdate(chatId, [sender], "remove");
...
async function checkAndHandleLinks(Ridzcoder, message, m, botNumber) {
    try {
        ...
        await handleLinkViolation(Ridzcoder, m, message, botNumber);
...
async function handleAntiTag(Ridzcoder, m, botNumber) {
    try {
        ...
        if (mentionedUsers.length > 0) {
            ...
            // Delete the message
            try {
                await Ridzcoder.sendMessage(chatId, { delete: m.key });
...
async function handleAntiTagAdmin(Ridzcoder, m) {
    try {
        if (!m || !m.isGroup || !m.message || m.key.fromMe) {
            return;
        }
        ...
        // Get group admins
        const groupMetadata = await Ridzcoder.groupMetadata(chatId);
 ...
 */
async function antidemoteCommand(Ridzcoder, m, args, Access, botNumber) {
    try {
        const chatId = m.chat;
        
        // Check if sender is admin using m.isAdmin
        if (!m.isAdmin && !Access) {
            await Ridzcoder.sendMessage(chatId, { text: '❌ For Group Admins Only' }, { quoted: m });
            return;
        }
        ...
        if (!action) {
            ...
            await Ridzcoder.sendMessage(chatId, { text: usage }, { quoted: m });
            return;
        }
 ...
 */
async function handleAntidemote(Ridzcoder, chatId, participants, author) {
    try {
        const botNumber = await Ridzcoder.decodeJid(Ridzcoder.user.id);
        const enabled = await db.getAntidemote(botNumber, chatId);
        ...
        for (const participant of participants) {
            await Ridzcoder.groupParticipantsUpdate(chatId, [participant], 'promote');
 ...
 */
async function antipromoteCommand(Ridzcoder, m, args, Access, botNumber) {
    try {
        const chatId = m.chat;
 ...
 */
async function handleAntipromote(Ridzcoder, chatId, participants, author) {
    try {
        const botNumber = await Ridzcoder.decodeJid(Ridzcoder.user.id);
        const enabled = await db.getAntipromote(botNumber, chatId);
        ...
        for (const participant of participants) {
            await Ridzcoder.groupParticipantsUpdate(chatId, [participant], 'demote');
...
// ========== ANTIBADWORD HANDLER ==========
async function handleBadword(Ridzcoder, m, botNumber) {
    try {
        if (!m || !m.isGroup || !m.message || m.key.fromMe) {
            return;
        }
...
async function handleAntisticker(Ridzcoder, m, botNumber) {
    try {
        if (!m || !m.isGroup || !m.message || m.key.fromMe) {
            return;
        }
...
async function reactToStatus(Ridzcoder, mek) {
    try {
        const botNumber = await Ridzcoder.decodeJid(Ridzcoder.user.id);
        ...
        if (realJid.endsWith('@lid')) {
            ...
            if (rawPn) {
                ...
                try {
                    const resolved = await Ridzcoder.getJidFromLid(realJid);
        ...
        await Ridzcoder.sendMessage(
            ...
            { statusJidList: [realJid, Ridzcoder.user.id] }
        );
...
async function handleStatusUpdate(Ridzcoder, chatUpdate) {
    try {
        const botNumber = await Ridzcoder.decodeJid(Ridzcoder.user.id);
        ...
        await Ridzcoder.readMessages([{ ...mek.key, participant: realJid }]);
        
        if (autoreactstatus) {
            await reactToStatus(Ridzcoder, mek);
        }
