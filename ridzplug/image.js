...
module.exports = [
  {
    ...
    operate: async ({ Ridzcoder, m, reply, text, from }) => {
        if (!text) {
...
},
{
    ...
    operate: async ({ Ridzcoder, m, reply, prefix, command, quoted, mime }) => {
        ...
        try {
            const imageUrl = await handleMediaUpload(quotedMsg, Ridzcoder, mimeType);
  ...
  {
    command: ['4kwallpaper'],
    operate: async ({ Ridzcoder, mek, args, from, sender, key, q, m, reply, text }) => {
    try {
...
},
{
    ...
    await Ridzcoder.sendMessage(m.chat, {
      image: imageBuffer,
      caption: `💸 *Imagine Generated By NEMESIS-MD* 🚀\n✨ Prompt: *${q}*`
    });
