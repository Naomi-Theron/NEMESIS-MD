    ...
    operate: async ({ ridzcoder, m, reply, args, prefix, Access, mess, moment }) => {
        ...
        try {           
            ...
            `╭──⧼♛ *NEMESIS MD GC CREATED*\n\n` +
                ...
                `│┃ ♛📅 *Created:* ${moment(created.creation * 1000).format("DD/MM/YYYY HH:mm:ss")}\n` +
            ...
            reply(`Failed to create group: ${error.message}`);
        }
    }
},
{
    ...
    operate: async ({ ridzcoder, m, reply, args, text, prefix, Access, mess }) => {
        ...
        const channelDesc = parts[1]?.trim() || '';
        
        if (!channelName) {
            return reply(`Example: ${prefix}createchannel JEXPLOIT FAMILY| This is my channel description`);
        }
        ...
        try {            
            ...
            if (!channelId) {
                ...
                console.warn('Warning: Channel ID not found in response');
            }
            ...
            reply(`Failed to create channel: ${error.message}`);
        }
    }
},
{
...
},
{
    ...
    operate: async ({ ridzcoder, m, reply, text, prefix, Access, mess }) => {
        try {
            ...
            if (!text) {
                return reply(`⚠️ Please provide a name!\n\nUsage: *${prefix}setprofilename <new_name>*\nExample: *${prefix}setprofilename My Awesome Bot*`);
            }

            // Limit name length to prevent errors
            if (text.length > 25) {
                return reply(`❌ Name too long! Maximum 25 characters allowed.`);
            }
            ...
            console.error('Error in setprofilename command:', error);
            reply(`❌ Failed to update profile name: ${error.message}`);
        }
    }
},
{
...
},
{
    ...
    operate: async ({ ridzcoder, m, reply, quoted, Access, mess }) => {
        ...
        if (!m.quoted) {
            return reply(`Reply to a user's message to block them.`);
        }
...
},
{
    ...
    operate: async ({ ridzcoder, m, reply, Access, mess, store }) => {
        ...
        try {
            ...
            const privateChats = allChats.filter(chat => {
                const id = chat.id || '';
                return id && !id.endsWith('@g.us') && id !== 'status@broadcast';
            });
            
            if (privateChats.length === 0) {
                return reply(`📭 *No private chats found.*`);
            }
            
            let message = `📋 *PRIVATE CHATS*\n`;
            message += `📊 *Total:* ${privateChats.length}\n\n`;
            
            for (let i = 0; i < Math.min(privateChats.length, 50); i++) {
                const chat = privateChats[i];
                const jid = chat.id;
                const number = jid?.split('@')[0] || 'Unknown';
                
                // Get name only - no messages
                let name = chat.name || chat.pushname || chat.notify || number;
            ...
            if (privateChats.length > 50) {
                message += `\n_...and ${privateChats.length - 50} more chats_`;
            }
            
            await reply(message);
            
        } catch (error) {
            console.error('List private chats error:', error);
